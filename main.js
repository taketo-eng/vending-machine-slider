...
class View {
    ...
    static createSlider() {
        config.sliderCon.classList.add("col-md-7", "col-12", "p-2", "d-flex", "justify-content-center", "align-items-center")

        config.sliderCon.innerHTML = `
            <div id='sliderShow' class="col-12 d-flex align-items-center">
...
class Algorithm {
    static chooseRotation(curr, input, length) {
        ...
        if ((distance < 0 && Math.abs(distance) > length / 2) || (distance > 0 && Math.abs(distance) < length / 2)) {
            return "left"
        } else return "right"
    }
}
